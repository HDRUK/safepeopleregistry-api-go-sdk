/*
Safe People Registry API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package safepeopleregistry-api-sdk

import (
	"encoding/json"
)

// checks if the Subsidiary type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Subsidiary{}

// Subsidiary Model representing subsidiaries
type Subsidiary struct {
	// Unique identifier for the subsidiary
	Id *int32 `json:"id,omitempty"`
	// Name of the subsidiary
	Name *string `json:"name,omitempty"`
	// Primary address line of the subsidiary
	Address1 *string `json:"address_1,omitempty"`
	// Secondary address line of the subsidiary
	Address2 *string `json:"address_2,omitempty"`
	// Town where the subsidiary is located
	Town *string `json:"town,omitempty"`
	// County where the subsidiary is located
	County *string `json:"county,omitempty"`
	// Country where the subsidiary is located
	Country *string `json:"country,omitempty"`
	// Postcode of the subsidiary
	Postcode *string `json:"postcode,omitempty"`
	// Website of the subsidiary
	Website *string `json:"website,omitempty"`
	// Indicates if the subsidiary is a parent company
	IsParent *int32 `json:"is_parent,omitempty"`
}

// NewSubsidiary instantiates a new Subsidiary object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSubsidiary() *Subsidiary {
	this := Subsidiary{}
	return &this
}

// NewSubsidiaryWithDefaults instantiates a new Subsidiary object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSubsidiaryWithDefaults() *Subsidiary {
	this := Subsidiary{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *Subsidiary) GetId() int32 {
	if o == nil || IsNil(o.Id) {
		var ret int32
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetIdOk() (*int32, bool) {
	if o == nil || IsNil(o.Id) {
		return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *Subsidiary) HasId() bool {
	if o != nil && !IsNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int32 and assigns it to the Id field.
func (o *Subsidiary) SetId(v int32) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *Subsidiary) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *Subsidiary) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *Subsidiary) SetName(v string) {
	o.Name = &v
}

// GetAddress1 returns the Address1 field value if set, zero value otherwise.
func (o *Subsidiary) GetAddress1() string {
	if o == nil || IsNil(o.Address1) {
		var ret string
		return ret
	}
	return *o.Address1
}

// GetAddress1Ok returns a tuple with the Address1 field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetAddress1Ok() (*string, bool) {
	if o == nil || IsNil(o.Address1) {
		return nil, false
	}
	return o.Address1, true
}

// HasAddress1 returns a boolean if a field has been set.
func (o *Subsidiary) HasAddress1() bool {
	if o != nil && !IsNil(o.Address1) {
		return true
	}

	return false
}

// SetAddress1 gets a reference to the given string and assigns it to the Address1 field.
func (o *Subsidiary) SetAddress1(v string) {
	o.Address1 = &v
}

// GetAddress2 returns the Address2 field value if set, zero value otherwise.
func (o *Subsidiary) GetAddress2() string {
	if o == nil || IsNil(o.Address2) {
		var ret string
		return ret
	}
	return *o.Address2
}

// GetAddress2Ok returns a tuple with the Address2 field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetAddress2Ok() (*string, bool) {
	if o == nil || IsNil(o.Address2) {
		return nil, false
	}
	return o.Address2, true
}

// HasAddress2 returns a boolean if a field has been set.
func (o *Subsidiary) HasAddress2() bool {
	if o != nil && !IsNil(o.Address2) {
		return true
	}

	return false
}

// SetAddress2 gets a reference to the given string and assigns it to the Address2 field.
func (o *Subsidiary) SetAddress2(v string) {
	o.Address2 = &v
}

// GetTown returns the Town field value if set, zero value otherwise.
func (o *Subsidiary) GetTown() string {
	if o == nil || IsNil(o.Town) {
		var ret string
		return ret
	}
	return *o.Town
}

// GetTownOk returns a tuple with the Town field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetTownOk() (*string, bool) {
	if o == nil || IsNil(o.Town) {
		return nil, false
	}
	return o.Town, true
}

// HasTown returns a boolean if a field has been set.
func (o *Subsidiary) HasTown() bool {
	if o != nil && !IsNil(o.Town) {
		return true
	}

	return false
}

// SetTown gets a reference to the given string and assigns it to the Town field.
func (o *Subsidiary) SetTown(v string) {
	o.Town = &v
}

// GetCounty returns the County field value if set, zero value otherwise.
func (o *Subsidiary) GetCounty() string {
	if o == nil || IsNil(o.County) {
		var ret string
		return ret
	}
	return *o.County
}

// GetCountyOk returns a tuple with the County field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetCountyOk() (*string, bool) {
	if o == nil || IsNil(o.County) {
		return nil, false
	}
	return o.County, true
}

// HasCounty returns a boolean if a field has been set.
func (o *Subsidiary) HasCounty() bool {
	if o != nil && !IsNil(o.County) {
		return true
	}

	return false
}

// SetCounty gets a reference to the given string and assigns it to the County field.
func (o *Subsidiary) SetCounty(v string) {
	o.County = &v
}

// GetCountry returns the Country field value if set, zero value otherwise.
func (o *Subsidiary) GetCountry() string {
	if o == nil || IsNil(o.Country) {
		var ret string
		return ret
	}
	return *o.Country
}

// GetCountryOk returns a tuple with the Country field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetCountryOk() (*string, bool) {
	if o == nil || IsNil(o.Country) {
		return nil, false
	}
	return o.Country, true
}

// HasCountry returns a boolean if a field has been set.
func (o *Subsidiary) HasCountry() bool {
	if o != nil && !IsNil(o.Country) {
		return true
	}

	return false
}

// SetCountry gets a reference to the given string and assigns it to the Country field.
func (o *Subsidiary) SetCountry(v string) {
	o.Country = &v
}

// GetPostcode returns the Postcode field value if set, zero value otherwise.
func (o *Subsidiary) GetPostcode() string {
	if o == nil || IsNil(o.Postcode) {
		var ret string
		return ret
	}
	return *o.Postcode
}

// GetPostcodeOk returns a tuple with the Postcode field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetPostcodeOk() (*string, bool) {
	if o == nil || IsNil(o.Postcode) {
		return nil, false
	}
	return o.Postcode, true
}

// HasPostcode returns a boolean if a field has been set.
func (o *Subsidiary) HasPostcode() bool {
	if o != nil && !IsNil(o.Postcode) {
		return true
	}

	return false
}

// SetPostcode gets a reference to the given string and assigns it to the Postcode field.
func (o *Subsidiary) SetPostcode(v string) {
	o.Postcode = &v
}

// GetWebsite returns the Website field value if set, zero value otherwise.
func (o *Subsidiary) GetWebsite() string {
	if o == nil || IsNil(o.Website) {
		var ret string
		return ret
	}
	return *o.Website
}

// GetWebsiteOk returns a tuple with the Website field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetWebsiteOk() (*string, bool) {
	if o == nil || IsNil(o.Website) {
		return nil, false
	}
	return o.Website, true
}

// HasWebsite returns a boolean if a field has been set.
func (o *Subsidiary) HasWebsite() bool {
	if o != nil && !IsNil(o.Website) {
		return true
	}

	return false
}

// SetWebsite gets a reference to the given string and assigns it to the Website field.
func (o *Subsidiary) SetWebsite(v string) {
	o.Website = &v
}

// GetIsParent returns the IsParent field value if set, zero value otherwise.
func (o *Subsidiary) GetIsParent() int32 {
	if o == nil || IsNil(o.IsParent) {
		var ret int32
		return ret
	}
	return *o.IsParent
}

// GetIsParentOk returns a tuple with the IsParent field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Subsidiary) GetIsParentOk() (*int32, bool) {
	if o == nil || IsNil(o.IsParent) {
		return nil, false
	}
	return o.IsParent, true
}

// HasIsParent returns a boolean if a field has been set.
func (o *Subsidiary) HasIsParent() bool {
	if o != nil && !IsNil(o.IsParent) {
		return true
	}

	return false
}

// SetIsParent gets a reference to the given int32 and assigns it to the IsParent field.
func (o *Subsidiary) SetIsParent(v int32) {
	o.IsParent = &v
}

func (o Subsidiary) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Subsidiary) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Address1) {
		toSerialize["address_1"] = o.Address1
	}
	if !IsNil(o.Address2) {
		toSerialize["address_2"] = o.Address2
	}
	if !IsNil(o.Town) {
		toSerialize["town"] = o.Town
	}
	if !IsNil(o.County) {
		toSerialize["county"] = o.County
	}
	if !IsNil(o.Country) {
		toSerialize["country"] = o.Country
	}
	if !IsNil(o.Postcode) {
		toSerialize["postcode"] = o.Postcode
	}
	if !IsNil(o.Website) {
		toSerialize["website"] = o.Website
	}
	if !IsNil(o.IsParent) {
		toSerialize["is_parent"] = o.IsParent
	}
	return toSerialize, nil
}

type NullableSubsidiary struct {
	value *Subsidiary
	isSet bool
}

func (v NullableSubsidiary) Get() *Subsidiary {
	return v.value
}

func (v *NullableSubsidiary) Set(val *Subsidiary) {
	v.value = val
	v.isSet = true
}

func (v NullableSubsidiary) IsSet() bool {
	return v.isSet
}

func (v *NullableSubsidiary) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSubsidiary(val *Subsidiary) *NullableSubsidiary {
	return &NullableSubsidiary{value: val, isSet: true}
}

func (v NullableSubsidiary) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSubsidiary) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


